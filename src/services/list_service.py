"""develop 정렬 리스트 생명주기: 목록/이름변경/삭제/확정/리포트 (P0 v3)."""
...
def _report(repo: Repo,list_id: UUID,principal: Principal)->dict:
 ...
 items=[]; links=[]
 for line in repo._all("SELECT pack_count,line_amount,snapshot FROM planning.purchase_line WHERE revision_id=%s ORDER BY created_at,id",(r["id"],)):
...
def confirm(conn,list_id:UUID,principal:Principal,*,name:str,planned_purchase_at:date|None,target_amount:int|None,memo:str)->dict:
 ...
 for x in candidates:
  ...
  repo._exec("INSERT INTO planning.purchase_line (revision_id,offer_id,selected_observation_id,pack_count,line_amount,snapshot) VALUES (%s,%s,%s,1,%s,%s)",(r["id"],x["offer_id"],x["observation_id"],int(x["price"]),Jsonb(s)))
