...
function renderAccountHeader(){const nav=$('.account-nav');if(!nav)return;const english=Boolean(window.TF_LOCALE?.isEnglish?.()),session=readAuthSession();if(!session){nav.innerHTML='<button type="button" data-auth="로그인">'+(english?'Sign in':'로그인')+'</button><button type="button" data-auth="회원가입">'+(english?'Sign up':'회원가입')+'</button>';bindAuthButtons();return}nav.innerHTML='<span class="account-user-name">'+esc(session.name||(english?'Member':'회원'))+(english?'':'님')+'</span><div class="service-menu mypage-menu"><button type="button" class="service-trigger" aria-haspopup="true">'+(english?'My page':'마이페이지')+'</button><div class="service-dropdown" aria-label="'+(english?'My page menu':'마이페이지 메뉴')+'"><a href="'+tfHref(tfPlanRoute())+'" data-account-basket>'+(english?'Basket':'장바구니')+'</a><a href="account.html">'+(english?'Edit profile':'회원정보 수정')+'</a><a href="#" data-account-logout>'+(english?'Sign out':'로그아웃')+'</a></div></div>';nav.querySelector('[data-account-logout]').onclick=async e=>{e.preventDefault();if(await tfLogout())location.reload()}}
