...
## 수정 제안 검토 뒤 추가 (2026-09-14 오후)

팀 제안("룰이 못 알아들은 문장만 LLM, 의도 분류 + 읽기 전용 답변 + 수치 가드")을 검토해 두 가지를 바로 넣었다.

- **수치·평가어 가드** (`_reply_within`·`_EVALUATIVE`): 답변의 숫자가 전부 입력(프롬프트·도구 결과·사용자 메시지)에
  있던 것이어야 하고, 부품 우열 평가어("뛰어나·우수·최고…")가 없어야 한다. 하나라도 걸리면 LLM 문장을 버리고
  코드 문장(`_guarded_reply`)으로: 바뀐 게 있으면 변경 목록+총액, 근거 질문이면 선조회 사실 그대로, 아니면 안내 문장.
  6턴 실측에서 오탐(정상 답변을 버림) 0건. 02 조건 에이전트엔 안 붙였다 — "150만원"↔`1500000` 정규화 불일치로
  오탐이 나고, 거기선 값 자체를 코드가 검사한다.
- **룰 경로 오탐 완화** (`_parse_swap_request`): "이 그래픽카드 **성능** 괜찮아?" 가 RTX 5070 Ti 로, "램은 **가성비**
  쪽이 나아?" 가 최저가로 **교체되던** 것. 질문 표지(`?·괜찮·나아·어때·맞아·일까·할까·뭐·어떤`)나 양방향어가 있으면
  실행하지 않고 "바꿔드릴까요? 바꾸려면 'GPU 더 좋은 걸로'" 로 되묻는다 — 제안의 "바꿔드릴까요? → 룰 동작".

제안 중 **"룰이 못 알아들은 문장만 LLM"** 순서는 위 오탐 때문에 반대했다 — 룰은 못 알아듣는 게 아니라 잘못 알아듣는다.
분류는 자유 텍스트 전부에 돌리고, 룰은 실행만 맡는 게 맞다. 행동 집합도 교체 하나가 아니라 지금 도구 6개(교체·시점·
수량·빼기/담기·설명)여야 "케이스는 나중에, 쿨러는 빼줘, GPU는 한 단계 위로" 같은 문장을 받는다. 파이널에서 "분류(행동
목록) → 룰 실행 → 읽기 전용 답변" 으로 수렴하면 지연이 2~3초 → 1초로 줄고 재현성이 오른다. `ResultSession` 의 실행
코드는 그대로 쓴다.
